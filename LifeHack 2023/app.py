...
def scam():
    ...
    return render_template("scam.html", username=username, user_id=user_id)

@app.route("/scamsuccess")
def scamsuccess():
    user_id = session.get("user_id")
    db.execute("UPDATE users SET banned=? WHERE id=?;", 1, user_id)
    session.clear()
    return redirect("/")
...
def failedloginscam():
    return render_template("failedloginscam.html")
...
def login():
    ...
    if request.method == "POST":
        ...
        #Error testing
        print(db.execute("SELECT banned FROM users WHERE username=?", username))
        if not username:
        ...
        if db.execute("SELECT banned FROM users WHERE username=?", username)[0]["banned"] == 1:
            return failedloginscam()
...
def profile():
    ...
    membershipList = membership.split(',')
    allFeeds = db.execute("SELECT * FROM posts;")
    return render_template("profile.html", groupList = groups, user_id=user_id, allFeeds=allFeeds, username=username)